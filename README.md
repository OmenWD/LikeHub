...
- Исходящий доступ к `https://<ваш-сервер>` по 443 порту

Внешних зависимостей нет: `manifest.json → requirements` пуст.

## Установка

1. HACS → Интеграции → ⋮ → Custom repositories → добавить репозиторий, тип `Integration`
2. Установить «LikeHub», перезапустить Home Assistant
3. Настройки → Устройства и службы → Добавить интеграцию → «LikeHub»
4. Ввести почту и пароль учётной записи сервиса
